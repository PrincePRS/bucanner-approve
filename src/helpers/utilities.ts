...
export function showFixedDecimalPoints(value: string, decimal: number): string {
  const roundValue =
    Math.round(parseFloat(value) * Math.pow(10, decimal)) /
    Math.pow(10, decimal);
  return roundValue.toFixed(decimal).replace(/\.?0+$/, "");
}
